...
V2 决策层修订（6 小时只加购块，**未冻结**）见 [`Q3_v2_revision_plan.md`](Q3_v2_revision_plan.md)；未过门前正式方案仍是本节 M1。
...
4. **光伏**：只用附件 3。`预报k小时` 对齐发布时间 \(T+k\) 整点功率，再线性插到 10 分钟**段初**。不用 Q2 的 XGB 或昨天光伏。
...
详见 [`Q3_v2_revision_plan.md`](Q3_v2_revision_plan.md)。要点：日内只改随后 36 格、禁止减购、6+12+18 目标不降不提交；结算 0.5/1.5/5 不变。实验写入 `results/Q3/v2/`，整包全年低于 M1 才替换 `result3.xlsx`。
