...
2. **时间**：144 个 10 分钟时段覆盖当天 0:00–24:00。附件时刻为**时段起点**（`00:10` 表示 `00:10–00:20`，`0:00+1` 表示次日 `00:00–00:10`）。电价循环对齐后第 1 段为附件最后一行。
...
8. **填表**：`result1.xlsx` **保留模板表头**。第一行 `0:10-0:20` 填日历第 2 段，最后一行 `0:00+1-0:10+1` 填日历第 1 段。论文表 1 的 `10:00–10:10` 取起点为 `10:00` 的时段。
...
- `*_kwh = *_kw / 6`。
- 四小时充放电聚合：日历段终点落在 `(0,4]`、`(4,8]`、…、`(20,24]` 的时段求和。
- `result1.xlsx` 保留官方「时间段」列，不改写成 `00:00-00:10` 起。
