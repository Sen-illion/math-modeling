...
附件里每个「时刻」只写一个钟点，表示 **10 分钟时段的起点**，不是瞬时点，也不是全天一个价。各问的代码、结算、结果表和论文必须用同一套读法。

- 一天 144 段，\(\Delta t=10\,\mathrm{min}=1/6\,\mathrm{h}\)。电量 = 功率 \(\times\Delta t\)。
- 表上的时刻 \(T\) 对应区间 \([T,\,T+10\mathrm{min})\)。例如 `00:10` = `00:10–00:20`，`00:20` = `00:20–00:30`，`23:50` = `23:50–24:00`。
- 表上的 `0:00` / `0:00+1`（次日 0:00）对应 **次日 `00:00–00:10`**。当天凌晨 `00:00–00:10` 的电价用同一行：附件 1 每天同一套 144 个数，把该行当作当天 0:00 开始的第一段。
- **附件最后一行才是当天 `00:00–00:10`。** 读入后须旋转/跨日拼接成日历序：`new[0]=old[-1]`，`new[1:]=old[:-1]`；年数据 `cal[d,0]=raw[d-1,-1]`（1 月 1 日午夜段用附件 1 典型日对齐后的第 0 段）。
- 优化与结算覆盖当天日历 `00:00–24:00`。官方模板**不改表头**：`0:10-0:20` 填日历 slot 1，最后一列 `0:00-0:10+1` 填次日 slot 0。
- 附件 1 的电价 \(\pi_t\) 是该时段的 **元/kWh**。少买 \(0.5\pi\)、多买 \(1.5\pi\)、紧急 \(5\pi\) 都用同一段的 \(\pi_t\)。
- 写入官方结果模板时，列序与表头对齐规则以该问 `planning/Qx_execution_plan.md` 为准，但不能改上述时刻含义。
- 细节见 `planning/modeling_notes.md`。改这条口径必须先改本条，再改代码和结果；当前已冻结的 Q1/Q2/Q3 数字若仍按「时刻=终点」算出，未重跑前不得改论文数字。
