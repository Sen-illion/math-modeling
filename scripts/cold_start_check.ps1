...
foreach ($relativeDirectory in $manifest.required_project_directories) {
  if (-not (Test-Path -LiteralPath (Join-Path $repoRoot ([string]$relativeDirectory)) -PathType Container)) { $missingProjectDirectories += [string]$relativeDirectory }
}
$selectedRoot = $null; $discoveredVersion = $null; $repositoryMatch = $false; $missingSkills = @(); $hashMismatches = @()
foreach ($candidate in $candidateRoots) {
  ...
  $currentMissing = @(); $hashMismatches = @(); foreach ($skillName in $manifest.required_skill_directories) { $skillFile = Join-Path $skillsRoot "$skillName\SKILL.md"; if (-not (Test-Path -LiteralPath $skillFile)) { $currentMissing += $skillName } elseif ($manifest.skill_file_hashes -and $manifest.skill_file_hashes.$skillName) { $text = [IO.File]::ReadAllText($skillFile, [Text.Encoding]::UTF8).Replace("`r`n", "`n").Replace("`r", "`n"); $sha = [Security.Cryptography.SHA256]::Create(); $actualHash = ([BitConverter]::ToString($sha.ComputeHash([Text.Encoding]::UTF8.GetBytes($text)))).Replace('-', ''); if ($actualHash -ne [string]$manifest.skill_file_hashes.$skillName) { $hashMismatches += $skillName } } }
  ...
  if ($repositoryMatch -and $discoveredVersion -eq [string]$manifest.plugin_version -and $missingSkills.Count -eq 0 -and $hashMismatches.Count -eq 0) { break }
}
$skillFound = $null -ne $selectedRoot; $versionMatch = $skillFound -and ($discoveredVersion -eq [string]$manifest.plugin_version); $hashPass = $hashMismatches.Count -eq 0; $projectStructurePass = $missingProjectDirectories.Count -eq 0; $skillPass = $skillFound -and $versionMatch -and $repositoryMatch -and $missingSkills.Count -eq 0 -and $hashPass; $overallPass = ($gitStatus -eq 'OK') -and $projectStructurePass -and $skillPass; $status = if ($overallPass) { 'PASS' } else { 'FAIL' }
New-Item -ItemType Directory -Path $localDir -Force | Out-Null
$state = [ordered]@{ schema_version=1; status=$status; verified_at=(Get-Date).ToString('o'); machine_local=$true; manifest_sha256=$manifestHash; required_source=$manifest.source; required_ref=$manifest.ref; required_plugin_version=$manifest.plugin_version; git=[ordered]@{status=$gitStatus; branch=$gitBranch; origin=$gitRemote}; project_structure=[ordered]@{pass=$projectStructurePass; missing_directories=$missingProjectDirectories}; skill=[ordered]@{found=$skillFound; path=$selectedRoot; discovered_version=$discoveredVersion; version_match=$versionMatch; repository_match=$repositoryMatch; missing_required_skills=$missingSkills; hash_mismatches=$hashMismatches; pinned_ref_verified_by_hash=$hashPass} }
