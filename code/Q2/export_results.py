...
def _series_for_headers(official: list[dict], day_idx: int, headers: list[str], key: str) -> np.ndarray:
    ...
    for i, header in enumerate(headers):
        ...
        if src is None:
            ...
            out[i] = float(np.asarray(src[key], dtype=float)[slot])
    return out
...
def export_result2(prices: pd.DataFrame, traces: list[dict], dest: Path | None = None) -> Path:
    ...
    if str(plan_headers[0]).replace(" ", "") != "0:10-0:20":
        raise ValueError(f"template first slot must stay 0:10-0:20, got {plan_headers[0]}")
